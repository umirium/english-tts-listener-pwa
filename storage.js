...
function createId() { if (window.crypto && crypto.randomUUID) return crypto.randomUUID(); return `id_${Date.now()}_${Math.random().toString(36).slice(2)}`; }
...
function findRecordByText(text) { const normalized = normalizeText(text); if (!normalized) return null; return getRecords().find(r => r.normalizedText === normalized) || null; }
